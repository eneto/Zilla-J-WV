...
public class AmenderTest extends TestCase {
	...
	public void testSetUpProduct(){
		try{
			...
			testProd.setName("JTest AmenderProduct");
			...
			testPrp.setName("JTest AmenderPlan");
			...
			testPrpc.setName("JTest AmenderCharge");
	...
	public void testSetUpSubscription() throws Exception {
		try{
			...
			acc.setCurrency("USD");
			acc.setName("JTest AmenderAccount");
			...
			QueryResult qpmres = zapi.zQuery("Select Id From PaymentMethod Where Name='Check'");
			ZObject[] zpms = qpmres.getRecords();
			PaymentMethod pm = (PaymentMethod) zpms[0];
			...
			subscription.setName("JTest AmenderSubscription");
			...
			//Set up RatePlanData
			QueryResult qprpres = zapi.zQuery("Select Id From ProductRatePlan Where Name='JTest AmenderPlan'");
	...
	public void testAddRatePlan() throws Exception {
		try{
			...
			QueryResult qres = zapi.zQuery("Select Id,Name From ProductRatePlan Where Name='JTest AmenderPlan'");
			ZObject[] zplans = qres.getRecords();
			ProductRatePlan prp = (ProductRatePlan) zplans[0];
			assertTrue(prp.getName().equals("JTest AmenderPlan"));

			QueryResult qresS = zapi.zQuery("Select Id,Name From Subscription Where Name='JTest AmenderSubscription' and Status='Active'");
			ZObject[] zsubs = qresS.getRecords();
			Subscription sub = (Subscription) zsubs[0];
			assertTrue(sub.getName().equals("JTest AmenderSubscription"));
	...
	public void testRemoveRatePlan() throws Exception {
		try{
			...
			assertTrue(rp.getName().equals("JTest AmenderPlan"));
			
			/* TEST */
			Amender amender = new Amender();
			
			//Test Preview Add Amendment
			AmenderResult amenderRes1 = amender.removeRatePlan("JTest AmenderAccount", rp.getId(), true);
			if(!amenderRes1.isSuccess()){
			...
			//Test Preview Add Amendment
			AmenderResult amenderRes2 = amender.removeRatePlan("JTest AmenderAccount", rp.getId(), false);
			if(!amenderRes2.isSuccess()){
				System.out.println(amenderRes2.getError());
			}
